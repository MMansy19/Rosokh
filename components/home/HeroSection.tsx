        ...
        <div className="text-center space-y-8">
          ...
          <div className="space-y-4 animate-slideInLeft">
            ...
            <p className="text-xl md:text-2xl text-muted max-w-3xl mx-auto leading-relaxed">
              {messages?.home?.subtitle ||
                "Your comprehensive Islamic multimedia platform"}
            </p>
          </div>{" "}
          {/* Description */}
          <div className="max-w-4xl mx-auto animate-slideInRight">
            <p className="text-lg md:text-xl text-muted leading-relaxed">
              {messages?.home?.description ||
                "Discover the beauty of Islamic knowledge through Quran reading, audio recitations, and spiritual growth tools."}
            </p>
          </div>
          {/* CTA Buttons */}
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-8 animate-fadeIn">
            ...
            >
              ...
              <span>
                {messages?.common?.actions?.read || "Read"}{" "}
                {messages?.common?.navigation?.quran || "Quran"}
              </span>
            ...
            >
              ...
              <span>
                {messages?.common?.actions?.listen || "Listen"}{" "}
                {messages?.common?.navigation?.audio || "Audio"}
              </span>
            </Link>
          </div>
          {/* Feature Cards Grid */}
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center space-y-3">
                    <div
                      className={`p-3 rounded-lg bg-primary/10 ${feature.color} group-hover:bg-primary/20 transition-colors duration-200`}
                    >
                      <Icon className="w-8 h-8" />
                    </div>{" "}
                    <h3 className="font-semibold text-foreground text-center">
                      {messages?.common?.navigation?.[feature.key] ||
                        feature.key}
                    </h3>
                    <p className="text-sm text-muted text-center leading-relaxed">
                      {messages?.home?.hero?.features?.[feature.key] ||
                        `Explore ${feature.key}`}
                    </p>
                  </div>
                </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-16 max-w-3xl mx-auto">
            <div className="text-center space-y-2 animate-fadeIn">
              ...
              <div className="text-muted">
                {messages?.home?.hero?.stats?.surahs || "Surahs"}
              </div>
            </div>
            <div
            ...
            >
              ...
              <div className="text-muted">
                {messages?.home?.hero?.stats?.verses || "Verses"}
              </div>
            </div>
            <div
              className="text-center space-y-2 animate-fadeIn"
            ...
            >
              ...
              <div className="text-muted">
                {messages?.home?.hero?.stats?.languages || "Languages"}
              </div>
            </div>
          </div>
        </div>
