      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          <div 
            ...
            role="status"
            aria-label={getTranslation(messages, 'audio.player.streamingReady', 'Streaming Ready')}
          ></div>
        ...
        <div className="flex flex-col">
          <span className="text-sm font-semibold text-foreground flex items-center gap-2">
            ...
            {getTranslation(messages, 'audio.player.audioPlayer', 'Audio Player')}
          </span>
          <span className="text-xs text-muted-foreground">
            {getTranslation(messages, 'audio.player.poweredByGoogleDrive', 'Powered by Google Drive')}
          </span>
        </div>
      </div>
        ...
        <button
          ...
          aria-label={getTranslation(messages, 'common.actions.close', 'Close player')}
        >
        ...
        <div className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 rounded-full border border-primary/20 backdrop-blur-sm">
          ...
          <span className="text-xs font-medium capitalize text-foreground">
            {getTranslation(messages, `audio.filters.quality.${track.quality}`, `${track.quality} Quality`)}
          </span>
        </div>
        ...
        <button 
          ...
          aria-label={`View ${track.title} on Google Drive`}
        >
