  ...
  const enhancedHandleIframeLoad = useMemo(() => {
    return () => {
      try {
        ...
        setError(
          `Failed to load audio player: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    };
  ...
  const enhancedHandleIframeError = useMemo(() => {
    return () => {
      try {
        ...
        setError(
          `Audio player encountered an error: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    };
