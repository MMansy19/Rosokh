...
export const QualityDistribution: React.FC<QualityDistributionProps> = React.memo(({ 
  qualities, 
  totalTracks, 
  locale, 
  messages 
  ...
  return (
    <section>
      <h4 className="font-medium text-foreground mb-3 flex items-center gap-2">
        <span role="img" aria-label="Audio quality">🎵</span>
        <span>{getTranslation(messages, 'audio.stats.qualityTitle', 'Audio Quality')}</span>
      </h4>
      <div 
        className="space-y-2"
        role="region"
        aria-label={getTranslation(messages, 'audio.stats.qualityLabel', 'Audio quality distribution')}
      >
        {qualityData.map(({ quality, count, percentage, colorClass, label }) => (
          <div 
            key={quality} 
