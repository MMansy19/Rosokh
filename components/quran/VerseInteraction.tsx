...
export function VerseInteraction({ 
  ...
  useEffect(() => {
    const savedActivities = localStorage.getItem(`verse_activities_${surahNumber}_${ayahNumber}`);
    if (savedActivities) {
      ...
      const memorizedActivity = activities.find((a: VerseActivity) => a.action === 'memorized');
  ...
  useEffect(() => {
    ...
    const verseElement = document.getElementById(`verse-${surahNumber}-${ayahNumber}`);
    if (verseElement) {
  ...
  const logVerseActivity = (action: VerseActivity['action'], duration?: number, notes?: string) => {
    ...
    const globalHistory = JSON.parse(localStorage.getItem('verse_reading_history') || '[]');
    globalHistory.push(activity);
    localStorage.setItem('verse_reading_history', JSON.stringify(globalHistory.slice(-1000))); // Keep last 1000 activities
  };
  ...
  const getActivitySummary = () => {
    ...
    const listenCount = verseActivities.filter(a => a.action === 'listened').length;
  ...
  return (
    ...
    >
      ...
      <div className="absolute -left-2 top-0 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {readCount > 0 && (
          <div className="w-2 h-2 bg-blue-500 rounded-full" title={`Read ${readCount} times`} />
        )}
        {listenCount > 0 && (
          <div className="w-2 h-2 bg-green-500 rounded-full" title={`Listened ${listenCount} times`} />
        )}
        {isMemorized && (
          <div className="w-2 h-2 bg-yellow-500 rounded-full" title="Memorized" />
        )}
        {verseActivities.some(a => a.action === 'reflected') && (
          <div className="w-2 h-2 bg-purple-500 rounded-full" title="Has reflection notes" />
        )}
      </div>
      ...
      {showNotes && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-border rounded-lg p-3 shadow-lg z-10">
          <textarea
            ...
            placeholder={
              locale === 'ar' ? 'أضف ملاحظاتك حول هذه الآية...' :
              locale === 'en' ? 'Add your reflection on this verse...' :
