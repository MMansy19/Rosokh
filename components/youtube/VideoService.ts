...
export class VideoService {
  ...
  async searchVideos(
    ...
    try {
      ...
      // Add duration filter
      if (filters.duration) {
        params.append('videoDuration', this.mapDurationFilter(filters.duration));
      }
   ...
   */
  async getFeaturedPlaylists(language: string = 'en'): Promise<VideoPlaylist[]> {
    try {
  ...
  async getTrendingVideos(region: string = 'US'): Promise<VideoMetadata[]> {
    try {
      ...
      const islamicVideos = data.items?.filter((item: any) => 
  ...
  private transformSearchResults(items: any[]): VideoMetadata[] {
    return items.map(item => ({
      ...
      quality: [{ resolution: '720p', url: `https://www.youtube.com/watch?v=${item.id.videoId || item.id}` }]
    }));
  }
  ...
  private isIslamicContent(snippet: any): boolean {
    ...
    const islamicKeywords = [
      'islam', 'islamic', 'muslim', 'quran', 'allah', 'prophet', 'muhammad',
      'إسلام', 'مسلم', 'قرآن', 'الله', 'محمد', 'نبي',
      'ислам', 'мусульманский', 'коран', 'аллах', 'пророк'
    ];
    ...
    const playlists: VideoPlaylist[] = [
      {
        ...
        title: language === 'ar' ? 'القرآن الكريم كاملاً' : 
               language === 'ru' ? 'Полный Коран' : 'Complete Quran Recitation',
      ...
      },
      {
        ...
        title: language === 'ar' ? 'محاضرات رمضان' : 
               language === 'ru' ? 'Лекции Рамадана' : 'Ramadan Lectures',
