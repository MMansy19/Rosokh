...
import { AudioTrack, Reciter } from "@/types/audio";
 ...
 */
export const useAudioData = (filters?: {
  reciter?: string;
  surah?: string;
}): useAudioDataReturn => {
  ...
  const loadData = useCallback(
    async (currentFilters?: { reciter?: string; surah?: string }) => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load audio data",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const refetch = useCallback(
    (newFilters?: { reciter?: string; surah?: string }) => {
  ...
  return {
    data:
      audioTracks.length > 0 || reciters.length > 0
        ? {
            tracks: audioTracks,
            reciters,
          }
