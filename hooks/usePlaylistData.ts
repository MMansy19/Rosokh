...
export const usePlaylistData = () => {
  ...
  const [loadedPlaylistVideos, setLoadedPlaylistVideos] = useState<
  ...
  const loadPlaylistVideos = async (
    ...
    try {
      ...
      const response = await fetch(
        `/data/youtube-playlists/${playlist.dataFile}`,
      );
      if (!response.ok) {
      ...
      if (!playlistData) {
        throw new Error(
          `Playlist data for ID ${playlistId} not found in ${playlist.dataFile}`,
        );
      }

      // Cache the videos
      setLoadedPlaylistVideos((prev) =>
        new Map(prev).set(playlistId, playlistData.videos),
      );
