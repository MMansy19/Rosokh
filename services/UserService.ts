...
export interface UserService {
  ...
  getListeningHistory(): Promise<Array<{ trackId: string; timestamp: string; duration: number }>>;
...
class UserServiceImpl extends BaseService implements UserService {
  ...
  async updatePreferences(preferences: Partial<UserPreferences>): Promise<UserPreferences> {
    const updatedPreferences = await this.fetchWithErrorHandling<UserPreferences>('/preferences', {
      method: 'PATCH',
  ...
  async getListeningHistory(): Promise<Array<{ trackId: string; timestamp: string; duration: number }>> {
    return await this.fetchWithErrorHandling('/history', {
      cacheKey: 'listening-history'
    });
  }

  async recordListeningSession(trackId: string, duration: number): Promise<void> {
    // Fire and forget - don't block UI for analytics
    this.fetchWithErrorHandling('/history', {
      method: 'POST',
      body: JSON.stringify({ trackId, duration, timestamp: new Date().toISOString() }),
      skipCache: true
    }).catch(error => {
  ...
  getUserMetrics() {
    ...
    return {
      ...
      cacheKeys: ['current-user', 'user-favorites', 'listening-history', 'user-preferences']
    };
  }
}
