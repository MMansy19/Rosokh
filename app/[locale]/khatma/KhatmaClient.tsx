...
export default function KhatmaClient({ locale, messages }: KhatmaClientProps) {
  ...
  const currentSession = khatmaSessions.find(
    (session) => session.id === selectedSession,
  );
  const sessionEntries = readingEntries.filter(
    (entry) => entry.sessionId === selectedSession,
  );

  const createNewSession = (
    sessionData: Omit<KhatmaSession, "id" | "currentProgress">,
  ) => {
        ...
        prev.map((session) =>
          session.id === selectedSession
            ? {
                ...
                status:
                  newProgress >= session.totalPages
                    ? "completed"
                    : session.status,
              }
            : session,
          ...
          <div className="text-center">
            ...
            <p className="text-lg text-muted max-w-2xl mx-auto mb-8">
              {messages?.khatma?.description ||
                "Track your Quran reading progress and complete your Khatma journey"}
            </p>

            <div className="card group rounded-lg shadow-lg p-8 max-w-md mx-auto">
              <h2 className="text-2xl font-bold text-foreground mb-4">
                {messages?.khatma?.startFirst || "Start Your First Khatma"}
              </h2>
              <p className="text-muted mb-6">
                {messages?.khatma?.startFirstDesc ||
                  "Begin your spiritual journey by setting up your first Quran reading session."}
              </p>
              <button
                onClick={() => setShowNewSessionForm(true)}
                className="w-full bg-surface hover:bg-buttonHover active:bg-buttonActive text-foreground py-3 px-6 rounded-lg transition-colors duration-200"
              >
    ...
    <div className="min-h-screen text-foreground transition-colors duration-300">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            onClick={() => setShowNewSessionForm(true)}
            className="bg-surface hover:bg-buttonHover active:bg-buttonActive text-foreground px-6 py-3 rounded-lg transition-colors duration-200"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  >
                    ...
                    <div className="mb-2">
                      <div className="flex justify-between text-sm text-muted mb-1">
                        <span>
                          {session.currentProgress} / {session.totalPages}{" "}
                          {messages?.khatma?.pages || "pages"}
                        </span>
                        <span>
                          {Math.round(
                      ...
                      <div className="w-full bg-surfaceChild rounded-full h-2">
                        <div
                          className="progress-bar h-2 rounded-full transition-all duration-300"
                          style={{
                            width: `${getProgressPercentage(session.currentProgress, session.totalPages)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="text-xs text-muted">
                      {messages?.khatma?.target || "Target"}:{" "}
                      {new Date(session.targetDate).toLocaleDateString(locale)}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-2">
            {currentSession ? (
              <div className="space-y-6">
                ...
                <div className="card group rounded-lg shadow-lg p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <p className="text-muted">
                        {messages?.khatma?.started || "Started"}:{" "}
                    ...
                    <button
                      ...
                      className="bg-surface hover:bg-buttonHover active:bg-buttonActive text-foreground px-4 py-2 rounded-lg transition-colors duration-200"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="text-center p-4 bg-surfaceChild rounded-lg">
                      <div className="text-2xl font-bold text-foreground">
                        {currentSession.totalPages -
                          currentSession.currentProgress}
                      </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-surfaceChild rounded-full h-3">
                      <div
                        className="progress-bar h-3 rounded-full transition-all duration-300"
                        style={{
                          width: `${getProgressPercentage(currentSession.currentProgress, currentSession.totalPages)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                ...
                <div className="card group rounded-lg shadow-lg p-6">
                  ...
                  {sessionEntries.length > 0 ? (
                          ...
                          >
                            <div>
                              <div className="font-semibold text-foreground">
                                {messages?.khatma?.surah || "Surah"}{" "}
                                {entry.surahFrom}:{entry.ayahFrom} -{" "}
                                {entry.surahTo}:{entry.ayahTo}
                              </div>
                              <div className="text-sm text-muted">
                    ...
                    <div className="text-center py-8 text-muted">
                      ...
                      <p>
                        {messages?.khatma?.noReadings ||
                          "No readings recorded yet. Start your first reading session!"}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="card group rounded-lg shadow-lg p-8 text-center">
                <div className="text-4xl mb-4">📚</div>
                <h3 className="text-xl font-bold text-foreground mb-4">
                  {messages?.khatma?.selectSession || "Select a Session"}
                </h3>
                <p className="text-muted">
                  {messages?.khatma?.selectSessionDesc ||
                    "Choose a Khatma session from the sidebar to view its details and add readings."}
                </p>
              </div>
            )}
          </div>
      ...
      <div className="card group rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              className="flex-1 px-4 py-2 bg-surface hover:bg-buttonHover active:bg-buttonActive text-foreground rounded-lg transition-colors duration-200"
            >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    surahFrom: parseInt(e.target.value),
                  }))
                }
                required
                min="1"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    ayahFrom: parseInt(e.target.value),
                  }))
                }
                required
                min="1"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    surahTo: parseInt(e.target.value),
                  }))
                }
                required
                min="1"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    ayahTo: parseInt(e.target.value),
                  }))
                }
                required
                min="1"
